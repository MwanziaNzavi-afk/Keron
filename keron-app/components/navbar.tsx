...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/15 bg-white/85 backdrop-blur-xl">
      <div className="container mx-auto flex items-center justify-between gap-3 px-4 py-2.5 sm:gap-6 sm:px-6 sm:py-4 xl:px-0">
        <Link href="/" className="site-logo-link shrink-0" aria-label="Keron Premier Properties home">
          <Image
            src="/images/keron-premier-logo.png"
            alt="Keron Premier Properties"
            width={565}
            height={313}
            className="site-logo rounded-md object-contain"
            priority
          />
        </Link>
        ...
        <nav className="hidden items-center gap-6 xl:flex">
          {navItems.map((item) => (
            <Link key={item.href} href={item.href} className="text-sm font-medium text-slate-700 transition hover:text-[#041d52]">
              {item.label}
            </Link>
          ))}
          <div className="relative">
            <button type="button" className="inline-flex items-center gap-1 text-sm font-medium text-slate-700 transition hover:text-[#041d52]" onClick={() => setShortStayOpen((isOpen) => !isOpen)} aria-expanded={shortStayOpen} aria-haspopup="menu">
              Short Stay
          ...
          <div className="relative">
            <button type="button" className="inline-flex items-center gap-1 text-sm font-medium text-slate-700 transition hover:text-[#041d52]" onClick={() => setServicesOpen((isOpen) => !isOpen)} aria-expanded={servicesOpen} aria-haspopup="menu">
              Services
            ...
            {servicesOpen ? (
              <div className="absolute left-1/2 top-full z-50 mt-4 w-80 -translate-x-1/2 rounded-2xl border border-slate-200 bg-white p-2 shadow-soft" role="menu">
                {serviceLinks.map((item) => (
                  <Link key={item.href} href={item.href} role="menuitem" className="block rounded-xl px-4 py-3 transition hover:bg-slate-50" onClick={() => setServicesOpen(false)}>
                    <span className="block text-sm font-semibold text-slate-950">{item.label}</span>
                    <span className="mt-1 block text-xs text-slate-500">{item.description}</span>
                  </Link>
                ))}
        ...
        <div className="hidden items-center gap-4 xl:flex">
          <Link href="/contact" className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-900 transition hover:border-[#041d52] hover:text-[#041d52]">
            <Phone className="h-4 w-4" />
            0708 669 141
          </Link>
        </div>

        <button type="button" className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-900 shadow-sm transition hover:border-slate-300 sm:h-11 sm:w-11 xl:hidden" onClick={() => setOpen(!open)} aria-label="Toggle navigation">
          {open ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}
        </button>
      </div>

      {open ? (
        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} className="overflow-hidden border-t border-white/80 bg-white/95 xl:hidden">
          <div className="flex flex-col gap-3 px-4 py-4 sm:px-6 sm:py-5">
            {navItems.map((item) => (
              <Link key={item.href} href={item.href} className="rounded-2xl px-4 py-3 text-sm font-medium text-slate-700 transition hover:bg-slate-100" onClick={() => setOpen(false)}>
                {item.label}
              </Link>
            ))}
            <div className="rounded-3xl bg-slate-50 p-2">
              ...
              {shortStayLinks.map((item) => (
                <Link key={item.href} href={item.href} className="block rounded-2xl px-3 py-2 text-sm font-medium text-slate-700 transition hover:bg-white" onClick={() => setOpen(false)}>{item.label}</Link>
              ))}
            </div>
            <div className="rounded-3xl bg-slate-50 p-2">
            ...
            <Link href="/contact" className="rounded-3xl px-4 py-3 text-sm font-medium text-[#041d52]" onClick={() => setOpen(false)}>Contact Sales</Link>
          </div>
        </motion.div>
      ) : null}
    </header>
